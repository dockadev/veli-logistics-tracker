        ...
        >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: '0.5rem' }}>
                <div className="depot-modal-item-title">
                    {getDepotDisplayName(depot)}
                </div>
                <span className={`badge ${depot.depotType === 'frontline' ? 'badge-crate-vehicle' : 'badge-item'}`} style={{ fontSize: '0.55rem', padding: '0.1rem 0.4rem', flexShrink: 0 }}>
                    {depot.depotType === 'frontline' ? (language === 'tr' ? 'Öncü' : 'Frontline') : (language === 'tr' ? 'Geri Hat' : 'Backline')}
                </span>
            </div>
...
export const DepotSelectionModal: React.FC<DepotSelectionModalProps> = React.memo(({
    ...
    const [hoveredRegion, setHoveredRegion] = useState<string>('All Depots');
    ...
    const { allRegionsList, depotsByRegion } = useMemo(() => {
        const grouped: Record<string, Depot[]> = {
            'all depots': Object.values(depots)
        };
        ...
        const customRegions = new Set<string>();

        Object.values(depots).forEach(depot => {
            const parts = depot.name.split(' - ');
            const prefix = parts.length > 0 ? parts[0].trim() : depot.name;
            const prefixLower = prefix.toLowerCase();

            if (!grouped[prefixLower]) {
                grouped[prefixLower] = [];
                customRegions.add(prefix);
            }
            grouped[prefixLower].push(depot);
        });

        const list = ['All Depots', ...FOXHOLE_REGIONS, ...Array.from(customRegions)];
        return { allRegionsList: list, depotsByRegion: grouped };
    }, [depots]);
    ...
    const searchResults = useMemo(() => {
        ...
        return Object.values(depots).filter(depot => {
            const displayName = getDepotDisplayName(depot).toLowerCase();
