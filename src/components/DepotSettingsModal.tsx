...
interface DepotSettingsModalProps {
    ...
    currentDepotType?: 'frontline' | 'backline';
    onSave: (accessCode: string, isCodePublic: boolean, depotType: 'frontline' | 'backline') => void;
    onClose: () => void;
}
...
export const DepotSettingsModal: React.FC<DepotSettingsModalProps> = React.memo(({
    ...
    const [depotType, setDepotType] = useState<'frontline' | 'backline'>('backline');
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setDepotType(currentDepotType || 'backline');
            setErrorMsg(null);
        }
    }, [isOpen, currentName, currentAccessCode, currentIsCodePublic, currentDepotType]);
    ...
    const handleSave = () => {
        ...
        onSave(accessCode, isCodePublic, depotType);
    };
            ...
            <div className="modal-wrapper" onClick={onClose}>
                ...
                >
                    <div className="modal-header">
                        <h3>{language === 'tr' ? 'Şifre Koy' : 'Set Password'}</h3>
                        <button className="modal-close" onClick={onClose}>
                            <X size={16} />
                        </button>
                    </div>
                    <div className="modal-body">
                        {/* We removed the custom identifier name group completely as per user request */}
